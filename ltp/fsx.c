...
test_swap_range(void)
{
	struct file_swap_range	fsr = {
		.file1_fd = fd,
		.flags = FILE_SWAP_RANGE_DRY_RUN | swap_flags,
	};
	int ret, e;

retry:
	ret = ioctl(fd, FISWAPRANGE, &fsr);
	e = ret < 0 ? errno : 0;
	if (e == EOPNOTSUPP && !(swap_flags & FILE_SWAP_RANGE_NONATOMIC)) {
		/*
		 * If the call fails with atomic mode, try again with non
		 * atomic mode.
		 */
		swap_flags = FILE_SWAP_RANGE_NONATOMIC;
