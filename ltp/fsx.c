...
int	exchrange_calls = 1;		/* -0 flag disables */
...
void
do_exchrange(unsigned offset, unsigned length, unsigned dest)
{
	struct xfs_exch_range	fsr = {
		.file1_fd = fd,
		.file1_offset = offset,
	...
	if (ioctl(fd, XFS_IOC_EXCHANGE_RANGE, &fsr) == -1) {
		prt("exchange range: 0x%x to 0x%x at 0x%x\n", offset,
				offset + length, dest);
		prterr("do_exchrange: XFS_IOC_EXCHANGE_RANGE");
		report_failure(161);
		goto out_free;
	}
...
have_op:
	...
	case OP_EXCHANGE_RANGE:
		...
		do_exchrange(offset, size, offset2);
		break;
	case OP_CLONE_RANGE:
		if (size == 0) {
...
main(int argc, char **argv)
{
	...
	if (exchrange_calls)
		exchrange_calls = test_exchrange();
